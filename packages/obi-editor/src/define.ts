// Registering the interface editor also registers the JSON editor it renders
// internally; importing the class alone still registers nothing.
import "@openbindings/json-editor/define";
import { defineElement } from "@openbindings/ui-core";
import { OBI_EDITOR_TAG, OBIEditorElement } from "./index.js";

defineElement(OBI_EDITOR_TAG, OBIEditorElement);

// Re-export the package surface so this entry ships full types (element
// class, HTMLElementTagNameMap and event-map augmentations), not just the
// registration side effect.
export * from "./index.js";
